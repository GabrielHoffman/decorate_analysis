...
```{r differential.histone.modification}
table(topTable(fitPC, coef='CellTypeNeuN+', number=Inf)$adj.P.Val < 0.05)
```
...
treeList = runOrderedClusteringGenome( quantResid, peakLocs2, method.corr="spearman" )   
```
...
treeListOriginal = runOrderedClusteringGenome( vobj$E, peakLocs2, method.corr="spearman" ) 
dfDistOriginal = evaluateCorrDecay( treeListOriginal, peakLocs2, "chr1" )
plotCorrDecay( dfDistOriginal, method="R", xlim=c(500, 1e6) )

# decay in residuals
dfDist = evaluateCorrDecay( treeList, peakLocs2, "chr1" )
...
```{r scatter, fig.width=15, fig.height=15, echo=TRUE, message=FALSE}
...
figlist = lapply(1:30, function(i){
